...
const Footer = ({
  ...
  return (
    <footer
      className={`relative py-12 lg:py-24 bg-orange-50 overflow-hidden ${className}`}
    >
      {backgroundImage &&
        ...
        <div className="flex flex-wrap mb-16 -mx-4">
          ...
          <div className="w-full md:w-7/12 lg:w-6/12 px-4 mb-16 lg:mb-0">
            <div className="flex flex-wrap -mx-4">
              {footerLinks.map((section) => (
                <div
                  key={section.title}
                  className="w-1/2 xs:w-1/3 px-4 mb-8 xs:mb-0"
                >
                  ...
                  <ul>
                    {section.links.map((link, index) => (
                      ...
                      >
                        <Link
                          href={link.href}
                          className="inline-block text-gray-600 hover:text-sky-500 font-medium transition-colors duration-200"
                        >
                          {link.label}
                        </Link>
                      </li>
          ...
          {showNewsletter && (
            <div className="w-full md:w-5/12 lg:w-4/12 px-4">
              <div className="max-w-sm p-8 bg-sky-900 rounded-2xl mx-auto md:mr-0">
                ...
                >
                  <fieldset>
                    ...
                    <input
                      ...
                      required
                      className="h-12 w-full px-4 py-1 bg-white placeholder-gray-700 outline-none ring-offset-0 focus:ring-2 focus:ring-sky-500 shadow rounded-full"
                    />
                    <Button
                      type="submit"
                      variant="primary"
                      className="h-12 text-sky-900 border-sky-500 hover:border-white bg-sky-500 hover:bg-white mt-3"
                    >
        ...
        <div className="flex flex-wrap -mb-3 justify-between items-center">
          {showSocial && (
            <div className="flex items-center mb-3 space-x-4">
              <a
                className="inline-block text-black hover:text-sky-500 transition-colors duration-200"
                href="#"
                aria-label="Facebook"
              >
                <Icon name="facebook" size={20} color="currentColor" />
