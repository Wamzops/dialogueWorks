    ...
    <div className="w-full lg:w-1/2 xl:w-7/12 px-4 mb-8 lg:mb-0">
      <div>
        ...
        <div className="mb-6">
          {post.tags &&
            post.tags.map((tag, index) => (
              <span
                key={index}
                className="inline-block mr-2 px-3 py-1 text-sm font-medium bg-sky-100 text-sky-700 rounded-full"
              >
                {tag}
              </span>
            ))}
        ...
        <Link href={`/blog/${post.slug}`} className="block group max-w-2xl">
          <h3 className="text-3xl font-medium group-hover:text-sky-600 transition duration-200 mb-4">
            {post.title}
          </h3>
          ...
          <div className="flex items-center">
            {post.author?.avatar ? (
              ...
              <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center font-bold text-primary text-xs">
                {(post.author?.name || post.author || "DW")[0]}
              </div>
            )}
